function outval = query_uservar(msgtxt, default)
%
...
%			3) a string
%
%	if default is a range of values, query_uservar will require the input
...
elseif length(default) == 1
	outtxt = sprintf('%s (%f)? ', msgtxt, default);
else
	outtxt = sprintf('%s [%f - %f](%f)? ', msgtxt, default(1), default(2), default(3));
end
